...
class Arr {
	 ...
	 * @return bool
	 */
	public static function has(array $arr, $key, $delimiter = '.') {
		$tmp = $arr;
		$config = explode ( $delimiter, trim ( $key, $delimiter ) );
		foreach ( ( array ) $config as $d ) {
			if (isset ( $tmp [$d] )) {
	 ...
	 * @return array
	 */
	public static function dot($array, $prepend = '', $delimiter = '.') {
		$results = [ ];
	 ...
	 * @return void
	 */
	public static function forget(&$array, $keys, $delimiter = '.') {
		$original = &$array;
		$keys = ( array ) $keys;
		if (count ( $keys ) === 0) {
			return;
